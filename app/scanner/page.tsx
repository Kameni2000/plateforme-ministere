...
export default function ScannerEntree() {
        ...
        async (texteDecode) => {
          ...
          try {
            ...
            if (membre) {
              ...
              // On vérifie s'il n'a pas déjà été bipé
              ...
              if (!dejaPresent) {
                // NOUVEAU : On enregistre avec l'e-mail du travailleur !
                await supabase.from('presences').insert([{
                  user_id: utilisateur.id,
                  membre_nom: membre.nom,
                  date_presence: aujourdHui,
                  scanne_par: utilisateur.email // Le nom de celui qui tient le téléphone
                }]);
              }
  ...
  const validerEntree = (membre: any) => {
    ...
    const nouvelleEntree = { ...membre, heure: new Date().toLocaleTimeString('fr-FR'), id_scan: Date.now() };
    setHistorique(prev => [nouvelleEntree, ...prev]);
    setTimeout(() => setDernierScan(null), 2500);
  };
      ...
      {dernierScan && (
        <div className="fixed inset-0 bg-emerald-500 z-50 flex flex-col items-center justify-center animate-in zoom-in duration-200">
          {dernierScan.photo_url ? (
            <img src={dernierScan.photo_url} alt={dernierScan.nom} className="w-48 h-48 rounded-full object-cover border-8 border-white mb-8 shadow-2xl" />
          ) : (
            <div className="w-40 h-40 bg-white rounded-full flex items-center justify-center text-8xl mb-8 shadow-2xl">✅</div>
          )}
          <h1 className="text-6xl font-black text-white uppercase tracking-widest text-center px-4">{dernierScan.nom}</h1>
          <p className="text-3xl font-bold text-emerald-900 mt-4 bg-white/20 px-8 py-2 rounded-full">⭐ MEMBRE VIP</p>
        </div>
      )}
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-clair/10 pb-6">
          ...
          <div className="bg-[#111] border border-clair/10 px-6 py-3 rounded-xl font-bold text-lg shadow-lg flex items-center gap-3">
            Mes Scans : <span className="text-or text-2xl font-black">{historique.length}</span>
          </div>
        </div>

        <div className="grid lg:grid-cols-2 gap-8">
          <div className="bg-[#111] p-6 rounded-3xl border border-clair/10 shadow-xl flex flex-col items-center">
            <h2 className="text-xl font-bold mb-4">Caméra Active</h2>
            ...
            {alerte && <div className="mt-4 bg-red-500/20 text-red-400 px-4 py-3 rounded-lg border border-red-500/50 w-full text-center font-bold animate-pulse">{alerte}</div>}
            
            <style jsx global>{`
              #lecteur-qr img { display: none !important; }
              #lecteur-qr button { background: #D4AF37 !important; color: #000 !important; font-weight: bold !important; border-radius: 8px !important; padding: 12px 24px !important; border: none !important; margin-top: 15px !important; cursor: pointer; }
          ...
          <div className="bg-[#111] p-6 rounded-3xl border border-clair/10 shadow-xl h-[600px] flex flex-col">
            <h2 className="text-xl font-bold mb-6 border-b border-clair/5 pb-4">Historique local</h2>
            <div className="flex-1 overflow-y-auto space-y-4 pr-2">
              {historique.map((entree) => (
                <div key={entree.id_scan} className="bg-sombre/80 p-4 rounded-xl border border-clair/5 flex justify-between items-center">
                  <div>
                    <h3 className="font-bold text-white">{entree.nom}</h3>
                  </div>
                  <div className="text-xs text-clair/40 font-mono bg-black px-2 py-1 rounded">{entree.heure}</div>
                </div>
              ))}
            </div>
          </div>
